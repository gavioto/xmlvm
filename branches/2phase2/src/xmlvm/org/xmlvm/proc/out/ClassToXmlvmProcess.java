...
public class ClassToXmlvmProcess extends XmlvmProcessImpl {
     ...
     */
    private static class ClassToXmlvmTask {
         ...
         * given Java class.
         * <p>
         * This class is a clone of the old ParseJVM. It should be cleaned
         * heavily.
         */
        class BcelVisitor extends EmptyVisitor {
            ...
            public void visitCode(Code code) {
                ...
                /*
                 * Pass 1: Give all referenced instruction handles a symbolic
                 * name, i.e. a label.
                 */
                ...
                for (int i = 0; i < ihs.length; i++) {
                    if (ihs[i] instanceof BranchHandle) {
                        ...
                        if (bi instanceof Select) { // Special cases
                            // LOOKUPSWITCH
                            // and
                            // TABLESWITCH
                ...
                for (int i = 0; i < lvs.length; i++) {
                    ...
                    /*
                     * Check if there is a gap between two variable
                     * declarations. If this is the case, it is an indication
                     * for a hidden variable.
                     */
                    ...
                    // lastOffset is 2 for computational type 2 otherwise offset
                    // is
                    // 1
                ...
                for (int i = 0; i < ihs.length; i++) {
                    ...
                    /*
                     * Iterate over catch-blocks in opposite order so that the
                     * catch-clauses end up in most-derived to base exception
                     * class.
                     */
                    ...
                    for (int j = 0; j < ehs.length; j++) {
                        if (ehs[j].getEndPC().equals(ih)) {
                            ...
                            if (inst instanceof MONITOREXIT
                                    && ehs[j].getStartPC().getPrev().getInstruction() instanceof MONITORENTER) {
                                xml_code.addContent(xml_inst);
                            }
                        }
                    }
            ...
            /**
             * The Java compiler sometimes generates additional arguments or
             * methods that are not in the original source code (called
             * <em>synthetic</em> arguments or methods. For those the compiler
             * does not generate local variable declarations (&lt;var&gt;). For
             * symmetry reasons, we generate those missing &lt;var&gt;
             * declarations in XMLVM.
             * 
             * @param xml_code
             * @param id
