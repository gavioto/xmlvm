...
public class RelativeLayout extends ViewGroup {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (isWrapContentHeight) {
            // Height already has top padding in it since it was calculated by
            // looking at
    ...
    private void measureChild(View child, LayoutParams params, int myWidth, int myHeight) {
        int childWidthMeasureSpec = getChildMeasureSpec(params.mLeft, params.mRight, params.width,
                params.leftMargin, params.rightMargin, paddingLeft, paddingRight, myWidth);
        int childHeightMeasureSpec = getChildMeasureSpec(params.mTop, params.mBottom,
                params.height, params.topMargin, params.bottomMargin, paddingTop, paddingBottom,
                myHeight);
     ...
     */
    private int getChildMeasureSpec(int childStart, int childEnd, int childSize, int startMargin,
            int endMargin, int startPadding, int endPadding, int mySize) {
        int childSpecMode = 0;
        int childSpecSize = 0;
    ...
    private void applyHorizontalSizeRules(LayoutParams childParams, int myWidth) {
        ...
        // left=10, right=-1 means the view must start at 10, but can go as far
        // as it wants to the right
        // left =-1, right=10 means the view must end at 10, but can go as far
        // as it wants to the left
        ...
        if (anchorParams != null) {
            childParams.mRight = anchorParams.mLeft
                    - (anchorParams.leftMargin + childParams.rightMargin);
        ...
        if (anchorParams != null) {
            childParams.mLeft = anchorParams.mRight
                    + (anchorParams.rightMargin + childParams.leftMargin);
    ...
    private void applyVerticalSizeRules(LayoutParams childParams, int myHeight) {
        ...
        if (anchorParams != null) {
            childParams.mBottom = anchorParams.mTop
                    - (anchorParams.topMargin + childParams.bottomMargin);
        ...
        if (anchorParams != null) {
            childParams.mTop = anchorParams.mBottom
                    + (anchorParams.bottomMargin + childParams.topMargin);
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0; i < count; i++) {
            View child = getChildAt(i);
            if (child.getVisibility() != GONE) {
                RelativeLayout.LayoutParams st = (RelativeLayout.LayoutParams) child
                        .getLayoutParams();
    ...
    public static class LayoutParams extends ViewGroup.MarginLayoutParams {
        ...
        public LayoutParams(Context c, AttributeSet attrs) {
            ...
            alignWithParent = attrs.getAttributeBooleanValue(null,
            ...
            mRules[ALIGN_BASELINE] = attrs.getAttributeResourceValue(null, "layout_alignBaseline",
                    0);
            ...
            mRules[ALIGN_PARENT_BOTTOM] = attrs.getAttributeBooleanValue(null,
                    "layout_alignParentBottom", false) ? TRUE : 0;
            mRules[CENTER_IN_PARENT] = attrs.getAttributeBooleanValue(null,
                    "layout_centerInParent", false) ? TRUE : 0;
            mRules[CENTER_HORIZONTAL] = attrs.getAttributeBooleanValue(null,
                    "layout_centerHorizontal", false) ? TRUE : 0;
            mRules[CENTER_VERTICAL] = attrs.getAttributeBooleanValue(null, "layout_centerVertical",
                    false) ? TRUE : 0;
    ...
    private static class DependencyGraph {
        ...
        private LinkedList<Node> findRoots(int[] rulesFilter) {
            ...
            // Builds up the dependents and dependencies for each node of the
            // graph
            for (int i = 0; i < count; i++) {
                final Node node = nodes.get(i);
                ...
                // Look only the the rules passed in parameter, this way we
                // build only the
        ...
        static class Node implements Poolable<Node> {
            ...
            private static final Pool<Node> sPool        = Pools.synchronizedPool(Pools.finitePool(
                                                                 new PoolableManager<Node>() {
