...
{
    staticInitializerLock(&__TIB_org_xmlvm_iphone_CGImage);

    // While the static initializer mutex is locked, locally store the value of
    // whether class initialization began or not
    int initBegan = __TIB_org_xmlvm_iphone_CGImage.classInitializationBegan;

    // Whether or not class initialization had already began, it has begun now
    __TIB_org_xmlvm_iphone_CGImage.classInitializationBegan = 1;

    staticInitializerUnlock(&__TIB_org_xmlvm_iphone_CGImage);

    JAVA_LONG curThreadId = (JAVA_LONG)pthread_self();
    if (initBegan) {
        if (__TIB_org_xmlvm_iphone_CGImage.initializerThreadId != curThreadId) {
            // Busy wait until the other thread finishes initializing this class
            while (!__TIB_org_xmlvm_iphone_CGImage.classInitialized) {
                // do nothing
            }
        }
    } else {
        __TIB_org_xmlvm_iphone_CGImage.initializerThreadId = curThreadId;
...
void __INIT_IMPL_org_xmlvm_iphone_CGImage()
{
    ...
    __TIB_org_xmlvm_iphone_CGImage.baseType = JAVA_NULL;
    __CLASS_org_xmlvm_iphone_CGImage_1ARRAY = XMLVM_CREATE_ARRAY_CLASS_OBJECT(__CLASS_org_xmlvm_iphone_CGImage);
    __CLASS_org_xmlvm_iphone_CGImage_2ARRAY = XMLVM_CREATE_ARRAY_CLASS_OBJECT(__CLASS_org_xmlvm_iphone_CGImage_1ARRAY);
    __CLASS_org_xmlvm_iphone_CGImage_3ARRAY = XMLVM_CREATE_ARRAY_CLASS_OBJECT(__CLASS_org_xmlvm_iphone_CGImage_2ARRAY);
