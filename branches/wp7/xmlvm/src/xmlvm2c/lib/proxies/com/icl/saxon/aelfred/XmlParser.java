...
loop:
	...
	if (type == ATTRIBUTE_ENUMERATED || type == ATTRIBUTE_NOTATION) {
	    enm = dataBufferToString ();
	}

	// Read the default value.
	requireWhitespace ();
	parseDefault (elementName, name, type, enm);
    }
	...
	if (tryRead ('#')) {
	    ...
	    value = readLiteral (flags);
	setAttribute (elementName, name, type, enm, value, valueType);
    }
	...
	if (stream != null) {
        ...
        try {
	        ...
	        error("Cannot read from " + systemId +
	            (systemId.equals(err.getMessage()) ? "" : " (" + err.getMessage() + ")"));
	    }
	}
