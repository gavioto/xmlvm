...
public class TextBlockRenderer extends JComponent {
    ...
    public Dimension getPreferredSize() {
        String text = textBlock.getText().value;
        Rectangle2D bounds = getFont().getStringBounds(text,
                new FontRenderContext(null, false, false));
        Dimension dimension = new Dimension((int) java.lang.Math.ceil(bounds.getWidth()),
                (int) java.lang.Math.ceil(bounds.getHeight()));
        return dimension;
    }

    private void updateFont() {
        setFont(new Font(textBlock.getFontFamily().getSource().value, Font.PLAIN,
