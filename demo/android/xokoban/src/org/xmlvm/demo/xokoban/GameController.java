...
public class GameController implements MoveFinishedHandler, Runnable {
	...
	if (adjacentBall != null
		&& (getBallAtPosition(newX + nextDX, newY + nextDY) != null || board
			.getBoardPiece(newX + nextDX, newY + nextDY) == Board.WALL)) {
	    return false;
	}
	...
	board = new Board(new CharField(Levels.getLevel(level),
	...
	DialogInterface.OnClickListener listener = new DialogInterface.OnClickListener() {
		...
		// level if necessary
		if (which == DialogInterface.BUTTON1 && !levelStarted
			&& currentLevel > 0) {
		    currentLevel--;
	...
	changeLevelDialog = new AlertDialog.Builder(gameView.getContext())
		.create();
	...
	congratulationDialog = new AlertDialog.Builder(gameView.getContext())
		.create();
	congratulationDialog
		.setTitle("Congratulations! All levels finished. Skipping to level 1.");
